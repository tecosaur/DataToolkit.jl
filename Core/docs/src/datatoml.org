...
The =driver= property is mandatory. =support= and =priority= can be omitted, in which
case they will adopt the default values. The default =support= value is either
