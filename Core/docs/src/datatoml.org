...
[[mydataset.TRANSFORMER]]
driver="transformer driver"
supports=["a QualifiedType", ...] # probably optional
supports="a QualifiedType" # single-value alternative form
priority=1 # (optional)
# other properties...
#+end_src
...
+ =supports=, a single ~QualifiedType~ string, or an array of them
