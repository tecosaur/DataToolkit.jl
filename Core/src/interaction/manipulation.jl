...
"""
    init(name::Union{AbstractString, Missing}, path::AbstractString;
         uuid::UUID=uuid4(), plugins::Vector{String}=DEFAULT_PLUGINS,
         quiet::Bool=false)
Create a new data collection at `path`, either a path to a .toml file or a
directory in which a Data.toml file should be placed. Any existing file is
overwritten. `name` will be guessed from `path` when `missing`.

Unless `quiet` is set, a message will be send to stderr reporting successful
creating of the data collection file.
"""
function init(name::Union{AbstractString, Missing}, path::AbstractString;
              uuid::UUID=uuid4(), plugins::Vector{String}=DEFAULT_PLUGINS,
              quiet::Bool=false)
    ...
    newcollection = DataCollection(LATEST_DATA_CONFIG_VERSION, name, uuid,
                                   ...
                                   path, DataAdviceAmalgamation(String[]),
                                   Main)
    write(newcollection)
    if !quiet
