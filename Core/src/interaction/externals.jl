...
```
"""
function storage(storer::S, as::Type; write::Bool=false) where {S <: DataStorage}
    if write && hasmethod(putstorage, Tuple{S, as})
        putstorage(storer, as)
    elseif hasmethod(getstorage, Tuple{S, as})
    ...
    for writer in potential_writers
        ...
        for storage in dataset.storage
            for write_fn_sig in write_fn_sigs
                supported_storage_types = filter(!isnothing, map(trytypeify, storage.type))::Vector{Type}
                valid_storage_types =
