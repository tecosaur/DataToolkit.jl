...
be downloaded from the web, and loaded as a CSV.
...
#+begin_note
We haven't installed the =CSV= package, but with the lazy-loading system, when
using the CSV loaded we are presented with the option to install it on-the-fly.
#+end_note
...
The headers are correct, but now the first line is being counted as part of the
data. This can be fixed by editing =iris= again and setting =args.skipto= to =2= in
the =csv= loader settings.

# The /Boston Housing/
# data set is part of the =RDatasets= package, and we can obtain a link to the raw
