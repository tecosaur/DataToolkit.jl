...
value of ~0.9~, and set the ~column~ to ~"MedV"~. It could be nice if we made those
...
We will then move the ~proportion = 0.9~ and ~column = "MedV"~ lines to the
arguments table.
...
Now, =edit= the =boston (clean)= dataset again and change the ~proportion~ to ~0.8~.
