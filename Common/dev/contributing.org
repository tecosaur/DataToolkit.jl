...
3. Decide whether you want to use an extra package, if so:
   ...
   e. Use ~@require {MyPkg}~ at the start of your [[@extref DataToolkitCore][~load~]] method implementation
   f. Add a ~@addpkg {MyPkg} {UUID}~ line to the ~__init__~ method in
      =src/DataToolkitCommon.jl= (maintaining the sorted order)
4. Implement one or more [[@extref DataToolkitCore][~load~]] methods for ~DataLoader{:name}~. Use [[@extref DataToolkitCore][~@getparam~]]
...
5. If you implemented multiple ~load~ methods, consider whether it would also be
   appropriate to implement a specialised [[@extref DataToolkitCore][~supportedtypes~]] method.
6. Consider whether there is a reasonable implementation of [[@extref DataToolkitCore][~createauto~]] you
   could write.
...
+ You want to implement either:
  - [[@extref DataToolkitCore][~storage~]]
  - [[@extref DataToolkitCore][~getstorage~]] and/or [[@extref DataToolkitCore][~putstorage~]]
+ Add ~"{name}"~ to ~DocStorage~ instead of ~DocSaveload~ in =docs/make.jl=

** Writer

The same as the loader steps, except:
+ You want to implement [[@extref DataToolkitCore][~save~]]
...
it's entirely likely this behaviour can be added in via a [[@extref DataToolkitCore "Plugins & Advice"][/Plugin/]].
