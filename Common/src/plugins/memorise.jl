...
"""
const MEMORISE_CACHE = Dict{Tuple{UUID, UUID, Type}, Pair{UInt, Any}}()
...
function memorise_read_a(f::typeof(DataToolkitCore.read1), dataset::DataSet, as::Type)
    ...
    stillvalid((lhash, info), checklhash) =
        lhash == checklhash && stillvalid(info)
    if should_memorise
        ...
        if haskey(MEMORISE_CACHE, dskey) && stillvalid(MEMORISE_CACHE[dskey], lhash)
            ...
            docache = function (info)
                MEMORISE_CACHE[dskey] = lhash => info
                info
            end
